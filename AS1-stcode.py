...
if first_page == 'Dashboards':
    ...
    if dash_no == 'First (Main) Dashboard':
        visual_type = st.selectbox('Select A Visualization:', 
                                    ['Factors of Mental Health', 'Mental Health vs Courses', 'Mental Health vs Relationship Status', 
                                     'Role of Social Support', 'Being Active vs Mental Health', 'Illness on Mental Health'])
        ...
        if visual_type == 'Factors of Mental Health':
            ...
            bar_chart = filtered_df[bar_att].value_counts()

            fig = px.bar(x=bar_chart.index, y=bar_chart.values, labels={'x': bar_att, 'y': filter_att}, template='plotly_dark')
            fig.update_layout(title="Correlation Between Mental Health Factors", xaxis_title=bar_att, yaxis_title=filter_att, height=600, width=800)
            st.plotly_chart(fig)

        # 2nd Visualization (Bar)
        elif visual_type == 'Mental Health vs Courses':
            bar_att = st.selectbox('Choose an attribute:', 
                               ['Stress_Level', 'Depression_Score', 'Anxiety_Score'])
            
            bar_chart = filteredbygender.groupby(['Course', bar_att]).size().reset_index(name='Count')

            fig = px.bar(bar_chart, x='Course', y='Count', color=bar_att, labels={'x': 'Course', 'y': 'Count'}, template='plotly_dark')
            fig.update_layout(title=f"Count for each {bar_att} in different courses", xaxis_title='Course', yaxis_title='Count', height=600, width=800)
            st.plotly_chart(fig)

        # 3rd Visualization (Bar)
        elif visual_type == 'Mental Health vs Relationship Status':
            bar_att = st.selectbox('Choose an attribute:', 
                               ['Stress_Level', 'Depression_Score', 'Anxiety_Score'])
            
            bar_chart = filteredbygender.groupby(['Relationship_Status', bar_att]).size().reset_index(name='Count')

            fig = px.bar(bar_chart, x='Relationship_Status', y='Count', color=bar_att, labels={'x': 'Relationship Status', 'y': 'Count'}, template='plotly_dark')
            fig.update_layout(title=f"Count for each {bar_att} in relationship status", xaxis_title='Relationship Status', yaxis_title='Count', height=600, width=800)
            st.plotly_chart(fig)

        # 4th Visualization (Area)
        elif visual_type == 'Role of Social Support':
        ...
        elif visual_type == 'Being Active vs Mental Health':
            area_att = st.selectbox('Choose an attribute:', 
                                    ['Stress_Level', 'Depression_Score', 'Anxiety_Score'])
        ...
        elif visual_type == 'Illness on Mental Health':
