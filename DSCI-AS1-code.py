...
if first_page == 'Dashboards':
    ...
    if dash_no == 'First (Main) Dashboard':
        ...
        gender_rad = st.radio('Select a Gender (applied to all) :', ['Male', 'Female'])
        ...
        fig = px.bar(x=bar_chart.index, y=bar_chart.values, color=bar_chart.index, labels={'x': bar_att1, 'y': filter_att})
        fig.update_layout(title="Correlation Between Mental Health Factors", xaxis_title=bar_att1, yaxis_title=filter_att, height=700, width=900)
        st.plotly_chart(fig)
        ...
        fig.update_layout(title=f"Count for each {bar_att2} in different courses", xaxis_title='Course', yaxis_title='Count', height=700, width=900)
        st.plotly_chart(fig)
        ...
        fig.update_layout(title=f"Count for each {bar_att3} in relationship status", xaxis_title='Relationship Status', yaxis_title='Count', height=700, width=900)
        st.plotly_chart(fig)
        ...
        fig = px.area(x=area_chart.index, y=area_chart.values, labels={'x': area_att, 'y': 'Count'})
        fig.update_layout(title=f"Count for each {area_att} in {category} category", xaxis_title=area_att, yaxis_title='Count', height=700, width=900)
        st.plotly_chart(fig)
        ...
        fig = px.area(x=area_chart.index, y=area_chart.values, labels={'x': area_att, 'y': 'Count'})
        fig.update_layout(title=f"Count for each {area_att} with {socsup_cat} Support", xaxis_title=area_att, yaxis_title='Count', height=700, width=900)
        st.plotly_chart(fig)
        ...
        fig = px.pie(names=['Yes', 'No'], values=[yes_total, no_total],
                title=f"{pie_att} vs {pie_cat}", labels={'names': pie_cat, 'values': 'Count'})
    ...
    if dash_no == 'Second Dashboard':
        ...
        fig.update_layout(xaxis_title='Age', yaxis_title='Count', height=700, width=900)
        st.plotly_chart(fig)
        ...
        fig.update_layout(xaxis_title=x_axis, yaxis_title=y_axis, height=700, width=900)
        st.plotly_chart(fig)
        ...
        fig.update_layout(xaxis_title='Course', yaxis_title='Count', height=700, width=900)
        st.plotly_chart(fig)
